...
const cache: {[row:string]: number} = {};

function trimStart(input: string): string {
  // remove leading . as long as there is a ? or # after it
  return input.startsWith(Condition.OPERATIONAL) ? input.split(/(?<=\.)(?=[^.])/).slice(1).join('') : input;
}
...
function determineArrangements(springs: string, groups: number[]): number {
  ...
  if(springs.length - groups.reduce((a,b) => a + b) - groups.length + 1 < 0) return 0;
  // Take a chunk of the row up to the first group size and see if it contains ? or #
  const damagedOrUnknown = !springs.slice(0, groups[0]).includes(Condition.OPERATIONAL);
  ...
  return cache[line] ??= (springs[0] != Condition.DAMAGED ? determineArrangements(trimStart(springs.slice(1)), groups) : 0) +
    (damagedOrUnknown && springs[groups[0]] != Condition.DAMAGED ? determineArrangements(trimStart(springs.slice(groups[0] + 1)), groups.slice(1)) : 0);
}
